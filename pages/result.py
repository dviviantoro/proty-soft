...
def update_prpd(chart, code, raw_source, raw_sensor, bgn_pos, bgn_neg, cycle):
    if bgn_pos == None : bgn_pos = 0
    if bgn_neg == None : bgn_neg = 0
    data_sensor = filter_and_align(raw_source, raw_sensor, bgn_pos, bgn_neg, cycle)
        ...
        with ui.element('div').classes('grid grid-cols-12 w-full gap-5 mt-16 mb-16'):
            with ui.element('div').classes('col-start-1 col-span-8 size-full'):
                ...
                with ui.tab_panels(tabs, value=tab_prpd).classes('w-full'):
                    ...
                    with ui.tab_panel(tab_prpd):
                        ...
                        """
                        # chart_prpd = ui.echart(options=create_dict_prpd(data_sine, data_sensor)).classes('h-[640px] w-full')
            ...
            with ui.card().classes('no-shadow col-start-9 col-span-4 size-full'):
                ...
                with ui.row().classes("w-full place-content-center"):
                    ui.button("check", color="#47C483", on_click=lambda: update_prpd(chart_prpd, summary, copied_source, copied_sensor, bgn_pos.value, bgn_neg.value, int(metadata["cycle"]))).classes("w-full")
